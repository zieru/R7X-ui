<template>
  <div>
    <a-card class="card" title="New Adjustment" :bordered="false">
      ...
      <repository-form :loading="loading" ref="repository" :showSubmit="false" />
    </a-card>
  </div>
</template>
...
<script>
    ...
    import RepositoryForm from '@/views/form/adjref/Adj2Form'
        ...
        methods: {
            ...
            validate () {
                ...
                Promise.all([repositoryForm]).then(values => {
                    this.$http.post('api/v1/adjustment', values[0]).then(data => {
                        if (data.message) {
